	...
	int value = player == 'x' ? -MINIMAX_MAX_UTILITY : MINIMAX_MAX_UTILITY;
		...
		if (player == 'x'){
			for (int i = 0; i < N_COLUMN; i++){
				if ( (aux = MAX(value,r_alfabeta(options[i], 1, -MINIMAX_MAX_UTILITY, MINIMAX_MAX_UTILITY, false)) ) < value ){
					value = aux;
					best_move = i;
					delete options[i];
				}
			...
			for (int i = 0; i < N_COLUMN; i++){
				if ( (aux = MIN(value,r_alfabeta(options[i], 1, -MINIMAX_MAX_UTILITY, MINIMAX_MAX_UTILITY, true)) ) < value ){
					value = aux;
					best_move = i;
					delete options[i];
				}
